        ...
        for batch in orden['orden']:
            if batch['status'] == 'open' and (batch['meat'] == self.tipos[0] or batch['meat'] == self.tipos[1]):
                ...
                if batch['quantity'] <= 5:
                    ...
                    # Le quito x tortillas, de los x tacos que vamos a preparar
                    ...
                    # por cada filling del batch, le quito x a ese filling // NEEDS CHECK
                ...
                # El batch es MAYOR A 5 TACOS
                    ...
                    # revisamos el tiempo que falta al taquero para descansar, y si llega a cero, se descansa
                    self.check_rest(5)
                    # Lo mismo con el abanico
                    fan_thread = Thread(target=self.fan_time, args=(5,))
                    fan_thread.start()

                    # Le quito 5 tortillas, de los 5 tacos que vamos a preparar
                    self.call_chalan("tortillas", 5)
                    self.fillings["tortillas"] -= 5

                    # por cada filling del batch, le quito 5 a ese filling
                    for ingredient in batch['ingredients']:
                    ...
                    batch['quantity'] -= 5
                    #  El tiempo****** que toma hacer los tacos
